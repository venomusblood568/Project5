...
class Spike(Object):
    def __init__(self, x, y, width, height, state="off"):
...
def handle_collisions(player, objects):
    for obj in objects:
        ...
        if obj.name == "spike" and pygame.sprite.collide_mask(player, obj):
            if player.y_vel > 0:
                player.rect.bottom = obj.rect.top
                player.landed()
                player.make_hit()
        ...
        if isinstance(obj, Block):  # Check for collision with floor objects
...
def main(window):
    ...
    spikes = [Spike(500, HEIGHT - block_size * 2, 16, 32),
              Spike(800, HEIGHT - block_size * 3, 16, 32, "on")]

    floor = [Block(i * block_size, HEIGHT - block_size, block_size) for i in range(-WIDTH // block_size, (WIDTH * 2) // block_size)]

    objects = [*floor, Block(0, HEIGHT - block_size * 2, block_size),
               Block(block_size * 3, HEIGHT - block_size * 4, block_size), fire, *spikes]
